...
customize settings in [[file:init.el][init.el]]. Custom.el is loaded before config.el (generated from config.org), so the clause to configure it, is included in [[init.el][init.el]].
