...
** Haskell
At the moment, we're only really investigating Haskell in conjunction with the "Integrated Haskell Platform", which itself carries a _lot_ of dependency baggage. At any rate, we need to be able to edit haskell with at least the minimum viable editing facilities...
#+begin_src emacs-lisp
  (use-package dante
    :straight t)
