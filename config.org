...
#+END_SRC
** colours on terminal
   custom colour themes generally enhance my expierience of writing
   inside emacs, but whenthat theme is applied to an instance running
   inside a terminal, the effect is really just terrible. This mode
   kind of approximates the effect in a graphical emacs frame.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Pretty symbols (experimenting with this. it might be annoying.)
#+BEGIN_SRC emacs-lisp
(when window-system
